...
export default async function BillPage({
  ...
  const whatsappMessage = [
    `Crown Accumulator - ${docTitle}`,
    `Document No: ${bill.billNumber}`,
    `Date: ${formatDate(bill.billDate)}`,
    bill.dueDate ? `Due Date: ${formatDate(bill.dueDate)}` : null,
    "",
    ...items.map(
      (item, i) =>
        `${i + 1}. ${item.description} — Qty: ${Number(item.quantity).toLocaleString()} × ${formatPKR(item.unitPrice)} = ${formatPKR(item.lineTotal)}`,
    ),
    "",
    `Subtotal: ${formatPKR(bill.subtotal)}`,
    Number(bill.taxAmount) > 0
      ? `Sales Tax (${Number(bill.taxRate)}%): ${formatPKR(bill.taxAmount)}`
      : null,
    Number(bill.sedAmount) > 0
      ? `S.E.D. (${Number(bill.sedRate)}%): ${formatPKR(bill.sedAmount)}`
      : null,
    Number(bill.shippingAmount) > 0
      ? `Shipping: ${formatPKR(bill.shippingAmount)}`
      : null,
    Number(bill.discountAmount) > 0
      ? `Discount: -${formatPKR(bill.discountAmount)}`
      : null,
    `*Net Total: ${formatPKR(bill.totalAmount)}*`,
    "",
    "Thank you for your business. Generated by Crown Accumulator Management System.",
  ]
    .filter(Boolean)
    .join("\n");
